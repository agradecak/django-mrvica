...
def nova_objava(request):
	objava_form = ObjavaForm(request.POST or None)
	...
	return render(request, 'app/nova_objava.html', {'objava_form': objava_form})

@login_required
def objava(request, pk):
	objava = Objava.objects.get(pk=pk)
	trenutni_path = request.path
	komentar_form = KomentarForm(request.POST or None)
...
def uredi_profil(request):
	...
	profil_form = ProfilForm(request.POST or None, instance=profil)
